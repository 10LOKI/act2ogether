...
try {
    ...
    $db->query("ALTER TABLE events ADD COLUMN image VARCHAR(255) NULL");
    
    echo "Image column added to events table!\n";
    ...
    // Show updated events
    $events = $db->fetchAll("SELECT title, image FROM events");
    foreach ($events as $event) {
        echo "- " . $event['title'] . " | " . ($event['image'] ?: 'No image') . "\n";
    }
